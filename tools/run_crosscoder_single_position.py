...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--intervention-step-manifest", type=Path, default=None,
        help="CSV with task_id and intervention_step; modify only that generation step.",
    )
...
def main() -> None:
    ...
    if args.intervention_step_manifest is not None:
        with args.intervention_step_manifest.open(newline="") as handle:
            step_rows = list(csv.DictReader(handle))
        intervention_steps = {str(row["task_id"]): int(row["intervention_step"]) for row in step_rows}
        if len(intervention_steps) != len(step_rows):
            raise ValueError("duplicate task IDs in intervention-step manifest")
        missing_steps = [str(row.get("task_id", "")) for row in rows if str(row.get("task_id", "")) not in intervention_steps]
        if missing_steps:
            raise KeyError(f"missing intervention steps: {missing_steps[:10]}")
        if args.token_scope != "last_token":
            raise ValueError("single-position manifest requires --token-scope last_token")
        if args.generation_backend == "hf_generate":
            raise ValueError("single-position manifest does not support hf_generate")
    ...
    for example_idx, row in enumerate(rows):
        ...
        intervention_step = (intervention_steps[str(row.get("task_id", ""))] if intervention_steps is not None else None)
        ...
        for _step in range(manual_steps):
            ...
            if args.alpha == 0.0:
                ...
                def hook(_module, _inputs, output):
                    ...
                    if intervention_step is not None and _step != intervention_step:
                        return output
        ...
        output_row.update(
            {
                ...
                "single_position_intervention_step": intervention_step,
