...
def main():
    ...
    for n,m in enumerate(manifest,1):
      ...
      try: lb=label[("deepseek_base",*key)]; lf=label[("deepseek_finetuned",*key)]
    ...
    for key,g in grouped.items():
      if set(g)!={"deepseek_base","deepseek_finetuned"}: skipped.append({"benchmark":key[0],"task_id":key[1],"reason":"missing source-text pair"}); continue
      pairs.append((key,g["deepseek_base"],g["deepseek_finetuned"]))
    ...
    for bench in sorted({x[0][0] for x in pairs}):
      ...
      for transition in ("regression","improvement"):
        if transition=="regression": pop=[x for x in bp if x[1]["base_label"]==0]; y=np.array([x[1]["finetuned_label"] for x in pop])
        else: pop=[x for x in bp if x[1]["base_label"]==1]; y=np.array([1-x[1]["finetuned_label"] for x in pop])
    ...
    # Steering candidates: direction consistent with the transition, repeated support, early evidence, and FT-side decoder.
    candidates=[]
    for transition in ("regression","improvement"):
      ...
      eligible.sort(key=lambda r:(r["effect_to_permutation_variability"],r["pr_lift"],r["positive_delta_support"],r["decoder_finetuned_specificity"]),reverse=True)
      seen=set()
      for r in eligible:
    ...
    for c in candidates:
      ...
      bp=[x for x in pairs if x[0][0]==bench]
      if transition=="regression": pop=[x for x in bp if x[1]["base_label"]==0 and x[1]["finetuned_label"]==1]
      else: pop=[x for x in bp if x[1]["base_label"]==1 and x[1]["finetuned_label"]==0]
      scored=[]
      ...
      for rank,(delta,task,vb,vf,tb,tf) in enumerate(sorted(scored,reverse=True)[:10],1):
        evidence.append({"transition":transition,"candidate_rank":c["candidate_rank"],"benchmark":bench,"feature_id":fid,"aggregation":stat,"evidence_rank":rank,"task_id":task,"base_text_score":vb,"finetuned_text_score":vf,"delta":delta,"base_text_tokens":tb,"finetuned_text_tokens":tf})
    if evidence:
    ...
    summary={"checkpoint":str(a.checkpoint),"step":ck["step"],"top_k":k,"manifest_rows":len(manifest),"encoded_rows":len(rows),"paired_tasks":len(pairs),"permutations":a.permutations,"seed":a.seed,"skipped":skipped,"labels":"1=fail; regression positive=base pass -> finetuned fail; improvement positive=base fail -> finetuned pass","score":"finetuned-source-text aggregate minus base-source-text aggregate","candidate_rule":"positive differential, ROC-AUC > .55, >= max(3,10% positive tasks) support; rank effect/permutation-null SD then PR lift/support/FT decoder specificity"}
