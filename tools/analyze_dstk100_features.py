...
def main():
 p=argparse.ArgumentParser();p.add_argument("--checkpoint",type=Path,required=True);p.add_argument("--activation-root",type=Path,required=True);p.add_argument("--labels",type=Path,required=True);p.add_argument("--output",type=Path,required=True);p.add_argument("--features",type=int,nargs="+",required=True);p.add_argument("--device",default="cuda:0");p.add_argument("--contexts",type=int,default=40);a=p.parse_args();a.output.mkdir(parents=True,exist_ok=True)
 ...
 tok=PreTrainedTokenizerFast.from_pretrained("JetBrains/deepseek-coder-6.7B-kexer",trust_remote_code=True,local_files_only=True)
 ...
 for ix,m in enumerate(manifest,1):
  xa=np.load(a.activation_root/m["benchmark"]/"deepseek_base"/m["filename"]);xb=np.load(a.activation_root/m["benchmark"]/"deepseek_finetuned"/m["filename"]);ids=xa["input_ids"]
