...
def prepare(project,run):
    ...
    semantic_subset={}
    if project=='codellama':
        semantic_subset['regression']={r['task_id'] for r in csv.DictReader(open('reports/codellama_base_merged_topk100_v1_regression_taxonomy/regression_failure_cases.csv')) if r['benchmark']=='bigcodebench' and r['primary_failure_category']=='wrong_logic_or_other_runtime'}
    else:
        semantic_subset['improvement']={json.loads(line)['task_id'] for line in open('runs/dstk100_continuous_f6404_f10168_f13801_v1/input.jsonl')}
    arms={}
    ...
    for tr,file in files.items():
        ...
        if tr in semantic_subset: wanted &= semantic_subset[tr]
        rows=[]
...
def main():
    ...
    run=Path('runs')/('eight_cell_top10_codellama_alpha3_semantic_v2' if a.project=='codellama' else 'eight_cell_top10_dstk_alpha3_semantic_v2')
