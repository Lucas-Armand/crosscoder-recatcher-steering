...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--intervention-token-manifest", type=Path, required=True, help="CSV with task_id and zero-based token_position in the provided prefix.")
    parser.add_argument("--no-special-tokens", action="store_true", help="Tokenize replay prefixes without BOS/EOS, matching attribution inputs.")
...
def main() -> None:
    ...
    with args.intervention_token_manifest.open(newline="") as handle:
        position_rows=list(csv.DictReader(handle))
    intervention_positions={str(row["task_id"]):int(row["token_position"]) for row in position_rows}
    if len(intervention_positions)!=len(position_rows):
        raise ValueError("duplicate task IDs in intervention-token manifest")
    missing=[str(row.get("task_id","")) for row in rows if str(row.get("task_id","")) not in intervention_positions]
    if missing: raise KeyError(f"missing intervention positions: {missing[:10]}")
    if args.token_scope!="last_token": raise ValueError("token-position runner requires --token-scope last_token")
    if args.generation_backend=="hf_generate": raise ValueError("token-position runner does not support hf_generate")
    ...
    for example_idx, row in enumerate(rows):
        ...
        intervention_position=intervention_positions[str(row.get("task_id",""))]
        ...
        encoded = tokenizer(prompt, return_tensors="pt", add_special_tokens=not args.no_special_tokens)
        ...
        for _step in range(manual_steps):
            ...
            if args.alpha == 0.0:
                ...
                def hook(_module, _inputs, output):
                    ...
                    if not 0 <= intervention_position < hidden.shape[1]:
                        raise IndexError(f"token position {intervention_position} outside prefix length {hidden.shape[1]}")
                    slice_start=intervention_position
                    slice_end=intervention_position+1
                    token_count=1
                    target_slice=hidden[:, slice_start:slice_end, :]
                    paired = None
                    if needs_reference:
                    ...
                    modified = hidden.clone()
                    modified[:, slice_start:slice_end, :] = (
                        target_slice + delta
                    )
        ...
        output_row.update(
            {
                ...
                "single_token_intervention_position": intervention_position,
