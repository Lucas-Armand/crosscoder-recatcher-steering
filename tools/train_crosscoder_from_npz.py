#!/usr/bin/env python3
"""
Train a simple sparse CrossCoder from paired ReCatcher activation NPZ files.
...
def load_layer_array(path: Path, layer_key: str) -> np.ndarray:
    # Auto-heal NPZ files whose activation layers were saved as dtype=object.
    # This converts all layer_* arrays in the local NPZ to float32 once, then
    # overwrites the local file atomically. Future reads of the same file are fast.
    ...
    if needs_rewrite:
        tmp_path = path.with_name(path.name + ".tmp_float32.npz")
        np.savez_compressed(tmp_path, **payload)
        tmp_path.replace(path)

    arr = payload[layer_key]
...
class ActivationPairBatcher:
    ...
    def _fill_buffer_once(self) -> None:
        ...
        for _ in range(max_attempts):
            ...
            arr_a = load_layer_array(path_a, self.layer_key)
            arr_b = load_layer_array(path_b, self.layer_key)
...
class CrossCoder(nn.Module):
    def __init__(self, dim_a: int, dim_b: int, latent_dim: int) -> None:
        super().__init__()
...
def split_train_val(
    ...
    n_val = max(1, n_val) if len(pairs) >= 10 and val_frac > 0 else n_val

    val = pairs[:n_val]
    train = pairs[n_val:] if n_val else pairs
...
def main() -> None:
    ...
    model = CrossCoder(dim_a=dim_a, dim_b=dim_b, latent_dim=args.latent_dim).to(device=device)
