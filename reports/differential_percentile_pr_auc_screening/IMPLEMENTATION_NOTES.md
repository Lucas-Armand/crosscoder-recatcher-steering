# Paired differential percentile PR-AUC screening implementation notes
...
For each side and feature, the analysis calculates P50, P60, P70, P80, P90,
P95, and P99 of the positive contribution over that side's exact
evaluated-token mask. Bias is excluded because it is shared and cancels in the
paired contrast. For percentile `P`, the task score is:

`delta_P = P_positive(c_variant) - P_positive(c_base)`.

Clamping precedes percentile calculation, so a zero P50 means that at least
half of evaluated-token contributions are nonpositive. Prompt and padding
tokens are excluded by the existing exact-mask loader.
...
positive normalized effect across every valid feature, all seven percentiles,
...
- `significant_feature_percentiles.csv`;
- `PERCENTILE_ANALYSIS.md`.
...
- `tools/run_differential_pr_auc_screening.py`;
- `tools/report_differential_percentiles.py`;
- `scripts/run_differential_pr_auc_screening.sh`;
- `tests/test_differential_pr_auc_screening.py`;
- `README.md`;
- `reports/differential_percentile_pr_auc_screening/IMPLEMENTATION_NOTES.md`.
