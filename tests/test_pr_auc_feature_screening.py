...
def test_pr_auc_is_tie_aware_for_both_positive_classes():
    values = np.array([[0.0], [1.0], [1.0], [2.0]], dtype=np.float32)
    labels = np.array([0, 0, 1, 1], dtype=np.int8)
    failure, success = pr_auc_both(*prepare_pr_order(values), labels)
    assert failure[0] == pytest.approx(5 / 6)
    assert success[0] == pytest.approx(5 / 12)


def test_normalized_pr_effect_uses_class_prevalence():
    observed = np.array([0.6, 0.2])
    actual = normalize_pr_effect(observed, 0.2)
    np.testing.assert_allclose(actual, [0.5, 0.0])
...
def test_end_to_end_smoke_outputs(tmp_path):
    ...
    script = Path(__file__).resolve().parents[1] / "tools" / "run_pr_auc_feature_screening.py"
    subprocess.run([
    ...
    assert len(list(output.glob("*/ranked_pr_auc_permutation_envelope.png"))) == 2
