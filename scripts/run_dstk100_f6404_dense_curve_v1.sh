...
run=runs/dstk100_f6404_dense_negative_curve_v1
...
common=(--checkpoint runs/crosscoder_deepseek_base_finetuned_layer16_same_text_topk100_v1/final.pt --model-a-id deepseek-ai/deepseek-coder-6.7b-base --model-b-id JetBrains/deepseek-coder-6.7B-kexer --target-side a --layer 16 --feature-id 6404 --intervention-mode topk_gated_suppression --token-scope last_token --generation-backend paired_cached --top-k 100 --rms-epsilon 1e-6 --input-jsonl "$run/input.jsonl" --max-new-tokens 512 --temperature 0.2 --top-p 0.95 --seed 1000 --device-a cuda:0 --device-b cuda:1 --dtype nf4 --reference-dtype float16 --tokenizer-id deepseek-ai/deepseek-coder-6.7b-base --trust-remote-code)
for spec in neg0p25:-0.25 neg0p75:-0.75 neg1p25:-1.25 neg1p5:-1.5 neg2p5:-2.5; do
  name=${spec%%:*}; alpha=${spec#*:}
  .venv/bin/python tools/run_crosscoder_intervention.py "${common[@]}" --alpha "$alpha" --output-jsonl "$run/generations/bigcodebench__f6404_alpha_${name}_results.jsonl" > "$run/logs/f6404_alpha_${name}.log" 2>&1
done
