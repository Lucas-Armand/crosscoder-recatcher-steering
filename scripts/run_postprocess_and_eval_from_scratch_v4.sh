...
DST_EXP="${DST_EXP:-crosscoder_final_dataset_v1_postprocessed_extraction_v4}"
...
WORK_ROOT="${WORK_ROOT:-/tmp/crosscoder_postprocess_and_eval_v4}"
...
COPY_ACTIVATIONS="${COPY_ACTIVATIONS:-0}"
...
This dataset is a extraction-v4 evaluation-ready copy.
...
Post-processing:
- extracts directly from prompt + raw_completion
- prefers a literal leading Python continuation over later fenced material
- selects the first Python fence, never the largest fence
- preserves helper definitions and cuts only explicit test/demo/explanation suffixes
- uses tools/reprocess_outputs_minimal.py
- records exact generated-character spans and extraction provenance
- applies only conservative formatting cleanup after extraction
...
for bench in $BENCHES_STR; do
  for model in $MODELS_STR; do
    ...
    rm -rf "$one"
    mkdir -p "${one}/in_results" "${one}/reprocessed"
    ...
    python tools/reprocess_outputs_minimal.py \
      --raw-results-dir "${one}/in_results" \
