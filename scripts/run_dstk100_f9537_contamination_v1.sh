...
run="runs/dstk100_f9537_test_contamination_v1"
...
cases = {
    r['task_id']
    for r in csv.DictReader(open(repo / 'reports/dstk100_transition_failure_analysis_v1/transition_failure_cases.csv'))
    if r['benchmark'] == 'bigcodebench'
    and r['transition'] == 'improvement'
    and r['primary_failure_category'] == 'generated_test_import_contamination'
}
...
out = []
for r in rows:
    if r['task_id'] not in cases or int(r.get('gen_idx', r.get('generation_idx', 0))) != 0:
        continue
...
p = repo / 'runs/dstk100_f9537_test_contamination_v1/input.jsonl'
...
arm() {
  ...
  .venv/bin/python tools/run_crosscoder_intervention.py "${common[@]}" \
    --alpha "$alpha" \
    --output-jsonl "$run/generations/bigcodebench__f9537_alpha_${name}_results.jsonl" \
...
r = Path('runs/dstk100_f9537_test_contamination_v1')
expected = [json.loads(x) for x in (r / 'input.jsonl').read_text().splitlines()]
observed = [json.loads(x) for x in (r / 'generations/bigcodebench__f9537_alpha_zero_results.jsonl').read_text().splitlines()]
checks = [
    {'task_id': x['task_id'], 'exact_raw_completion': x['raw_completion'] == y['raw_completion']}
    for x, y in zip(expected, observed)
]
(r / 'BASELINE_REPRODUCTION.json').write_text(json.dumps(checks, indent=2) + '\n')
