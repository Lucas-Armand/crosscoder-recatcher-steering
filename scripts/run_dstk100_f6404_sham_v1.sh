...
run=runs/dstk100_f6404_sham_orthogonal_v1
...
common=(--checkpoint runs/crosscoder_deepseek_base_finetuned_layer16_same_text_topk100_v1/final.pt --model-a-id deepseek-ai/deepseek-coder-6.7b-base --model-b-id JetBrains/deepseek-coder-6.7B-kexer --target-side a --layer 16 --feature-id 6404 --per-example-direction-npz "$run/sham_orthogonal_seed6404.npz" --preserve-per-example-direction-norm --intervention-mode topk_gated_suppression --token-scope last_token --generation-backend paired_cached --top-k 100 --rms-epsilon 1e-6 --input-jsonl "$run/input.jsonl" --max-new-tokens 512 --temperature 0.2 --top-p 0.95 --seed 1000 --device-a cuda:0 --device-b cuda:1 --dtype nf4 --reference-dtype float16 --tokenizer-id deepseek-ai/deepseek-coder-6.7b-base --trust-remote-code)
for spec in neg0p5:-0.5 neg1:-1 neg2:-2; do
  name=${spec%%:*}; alpha=${spec#*:}
  .venv/bin/python tools/run_crosscoder_intervention.py "${common[@]}" --alpha "$alpha" --output-jsonl "$run/generations/bigcodebench__sham_${name}_results.jsonl" > "$run/logs/sham_${name}.log" 2>&1
done
